...
vec p_fitted( const vec &beta, const mat &w, const mat &alpha, const mat &x, const mat &Z, const uvec &index, vec &prob )
{
  mat eta = Z * w;
  vec f   = vectorise (eta+alpha);
  
  unsigned int N = x.n_rows;
  vec linear_predictor = x * beta; 
  for ( unsigned int i=0 ; i<N ; i++ ) {
    linear_predictor(i) += f(index(i)-1);
  }
  
  prob = 1.0/( 1.0+arma::exp(-linear_predictor) );
...
double log_post(const uvec &y, const vec &lp, const vec &prob, const vec &beta, double beta_var, const mat &alpha, 
...
)
{
  ...
  unsigned int n = x.n_rows;
  vec p_current(n);
  vec lp_current = p_fitted(beta, w, alpha, x, Z, index, p_current);
  ...
  vec p_proposed(n);
  vec lp_proposed = p_fitted(beta_new, w_new, alpha_new, x, Z, index, p_proposed );
  ...
  /* MH ratio */
  /* Likelihoods */
  double post_current  = log_post(y, lp_current, p_current, beta, 10.0, alpha, sigma_alpha, w, w0, S_inv, tau );
  double post_proposed = log_post(y, lp_proposed, p_proposed, beta_new, 10.0, alpha_new, sigma_alpha, w_new, w0, S_inv, tau );
  double R = post_proposed-post_current;
  /* Correction */
