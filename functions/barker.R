...
barker_update = function( beta, alpha, w_i, step_size, x, dataset, Z, sigma2_alpha, S_inv, w0, tau_i, index ) {
  ...
  logit_prob <- 1 / (1 + exp(-beta_x * noise))  
  
  # Generate b using vectorized 2*Bernoulli - 1 trick
  b <- 2 * rbinom(length(logit_prob), size = 1, prob = logit_prob) - 1
